...
@dataclass
class AliasedPerson:
    first_name: Annotated[str, MuName("display-name")]
...
class VariadicCfg:
    name: str
    tags: Annotated[list[str], MuZeroOrMore]
    features: Annotated[list[str], MuOneOrMore]


@dataclass
class OptionalCfg:
    name: str
    nickname: Annotated[str | None, MuOptional]
...
class FieldOverrideCfg:
    value: Annotated[int, MuDeserialize(lambda _expr, _ctx: 7)]
...
def test_union_tag_dispatch() -> None:
    result = parse_one_typed('(app-jvm "demo" :main "demo.Main")', AppJvm | AppPy)
    assert isinstance(result, AppJvm)
    assert result == AppJvm(name="demo", main="demo.Main")


def test_union_tag_override() -> None:
    result = parse_one_typed('(custom-app "x")', CustomTagApp | AppJvm)
...
def test_union_unknown_tag_fails() -> None:
    with pytest.raises(MuDecodeError):
        parse_one_typed('(unknown-tag "x")', AppJvm | AppPy)


def test_field_name_default_snake_to_kebab() -> None:
    result = parse_one_typed('(person :first-name "alice" :age 32)', Person)
    assert result == Person(first_name="alice", age=32)


def test_field_name_alias_override() -> None:
    result = parse_one_typed('(aliased-person :display-name "alice")', AliasedPerson)
...
def test_one_or_more_missing_fails() -> None:
    with pytest.raises(MuDecodeError):
        parse_one_typed('(variadic-cfg "demo")', VariadicCfg)


def test_optional_marker_sets_none_when_omitted() -> None:
    result = parse_one_typed('(optional-cfg "demo")', OptionalCfg)
...
def test_custom_deserializer_precedence_field_over_registry() -> None:
    ...
    def reg_decoder(_expr, _ctx: MuDecodeContext) -> int:
        return 999

    registry.register(int, reg_decoder)
    result = parse_one_typed('(field-override-cfg 123)', FieldOverrideCfg, registry=registry)
...
def test_registry_custom_deserializer_used_when_no_field_override() -> None:
    ...
    def decode_int(expr, _ctx: MuDecodeContext) -> int:
        from mu.types import SAtom

        assert isinstance(expr, SAtom)
        return len(expr.value)

    registry.register(int, decode_int)
    result = parse_one_typed('(registry-cfg 12345)', RegistryCfg, registry=registry)
...
def test_recursive_nested_decode() -> None:
    ...
    """
    result = parse_one_typed(source, Root)
    assert result == Root(
        child=Child(value=1),
        items=[1, 2, 3],
        flags={"enabled": True},
